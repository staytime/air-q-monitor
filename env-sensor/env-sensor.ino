...
static const TIMECOUNTER_T TS_BLCOK_CCS811_ENV = 5 * TS_1_SECOND;
static const TIMECOUNTER_T TS_OFFSET_CCS811_ENV = 1 * TS_1_SECOND;

static const TIMECOUNTER_T TS_BLCOK_CCS811_UPDATING = 2 * TS_1_SECOND;
static const TIMECOUNTER_T TS_OFFSET_CCS811_UPDATING = 1 * TS_1_SECOND + 1;

static const TIMECOUNTER_T TS_BLCOK_CCS811_BASELINE = 60 * 60 * TS_1_SECOND;
static const TIMECOUNTER_T TS_OFFSET_CCS811_BASELINE = 10 * 60 * TS_1_SECOND;


static const TIMECOUNTER_T TS_CYCLE_SIZE = TS_BLOCK_HYGROMETER * TS_BLCOK_CCS811_ENV * TS_BLCOK_CCS811_UPDATING * TS_BLCOK_CCS811_BASELINE;
...
void setup() {
  ...
  timerAlarmWrite(wdt_timer, WDT_TIMEOUT, false); //set time in us
  ...
  for(int i = 0;i < 5 & (!ccs.available()); ++i) {
    Serial.println("wait 1 second for ccs811");
    timerWrite(wdt_timer, 0);
    delay(1000);
  }
  while (!ccs.available()) {
...
void loop() {
  ...
  if (time_share_check(TS_BLCOK_CCS811_ENV, TS_OFFSET_CCS811_ENV)) {
    Serial.printf("update env for ccs811 %f %f\n", lastest_state.humidity.to_float(), lastest_state.temperature.to_float());
  ...
  if (time_share_check(TS_BLCOK_CCS811_UPDATING, TS_OFFSET_CCS811_UPDATING)) {
    if(ccs.available()){
      if(!ccs.readData()){
        ...
        pDataReportCharacteristic->setValue(bleDataReportSerData, 14);
        pDataReportCharacteristic->notify();
      }
    }
  }

  if(time_share_check(TS_BLCOK_CCS811_BASELINE, TS_OFFSET_CCS811_BASELINE)) {
